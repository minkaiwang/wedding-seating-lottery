...
import { Languages, Maximize, Minimize, TabletSmartphone } from 'lucide-vue-next'
...
const globalConfigStore = useStore().globalConfig
const {
    getServerStatus: serverStatus,
} = storeToRefs(serverConfig)
const { getLanguage } = storeToRefs(globalConfigStore)
...
const languageLabel = computed(() => (getLanguage.value === 'en' ? 'EN' : '中'))

function toggleLanguage() {
    const next = getLanguage.value === 'en' ? 'zhCn' : 'en'
    globalConfigStore.setLanguage(next)
}
...
<template>
  <div v-if="visible" ref="settingRef" class="flex flex-col gap-3">
    ...
    <div class="tooltip tooltip-left" :data-tip="t('tooltip.switchLanguage')">
      <div
        class="flex items-center justify-center w-10 h-10 p-0 m-0 cursor-pointer setting-container bg-slate-500/50 rounded-l-xl hover:bg-slate-500/80 hover:text-blue-400/90 text-xs font-bold tracking-wide"
        @click="toggleLanguage"
      >
        <Languages class="absolute h-4 w-4 opacity-40" />
        <span class="relative z-[1]">{{ languageLabel }}</span>
      </div>
    </div>
