...
const { addPrize, resetDefault, resetPrizeProgress, delAll, delItem, prizeList, currentPrize, selectedPrize, submitData, changePrizePerson, changePrizeStatus, selectPrize, localImageList } = usePrizeConfig()
const { t } = useI18n()
</script>
    ...
    <PageHeader :title="t('viewTitle.prizeManagement')">
      <template #buttons>
        <div class="flex w-full gap-3">
          ...
          <button class="btn btn-warning btn-sm" @click="resetPrizeProgress">
            {{ t('button.resetPrizeProgress') }}
          </button>
      ...
      >
        ...
        <label class="flex items-center w-1/2 max-w-xs gap-2 form-control">
          <div class="label">
            ...
            <HoverTip :tip="t('tooltip.prizeRepeatable')" />
          </div>
          <input
