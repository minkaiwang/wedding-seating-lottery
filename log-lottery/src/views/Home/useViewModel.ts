...
/** Camera distance for sphere (rolling ball) home view. */
export const HOME_SPHERE_CAMERA_Z = 3000
...
export function useViewModel() {
    ...
    function initThreeJs() {
        ...
        renderer.value.setSize(width, height * 0.96)
        ...
        const homeLayout = computeHomeLayoutMetrics(tableData.value, rowCount.value, cardSize.value)
        ...
        // 创建横铺的界面
        const tableVertices = createTableVertices({
            tableData: tableData.value,
            rowCount: rowCount.value,
            layout: homeLayout,
        })
        targets.table = tableVertices
        cameraZ.value = computeHomeTableCameraZ(homeLayout, rowCount.value, { width, height })
        const cameraYOffset = computeHomeCameraYOffset(homeLayout)
        camera.value.position.z = cameraZ.value
        camera.value.position.y = cameraYOffset
        if (controls.value) {
            controls.value.target.set(0, cameraYOffset, 0)
        }
        // 创建球体
        const sphereVertices = createSphereVertices({ objectsLength: objects.value.length })
        targets.sphere = sphereVertices
        window.addEventListener('resize', onWindowResize, false)
        void activateSphereView({ animate: tableLen > 0 })
        render()
    }

    function applySphereCamera() {
        cameraZ.value = HOME_SPHERE_CAMERA_Z
        camera.value.position.z = HOME_SPHERE_CAMERA_Z
    ...
    async function activateSphereView(options: { animate?: boolean } = {}) {
        ...
        if (animate && targets.sphere.length > 0) {
            await transform(targets.sphere, 1200)
        }
        else if (targets.sphere.length > 0) {
    ...
    async function enterLottery() {
        ...
        await activateSphereView({ animate: true })
    }
    /**
     * @description 开始抽奖
     */
    function startLottery() {
        ...
        prizeConfig.ensureCurrentPrize()
        // 验证是否已抽完全部奖项
        if (currentPrize.value.isUsed || !currentPrize.value?.id) {
            toast.open({
    ...
    async function continueLottery() {
        ...
        luckyCount.value = 0
        prizeConfig.completeDrawRound(currentPrize.value)
        personConfig.addAlreadyPersonList(luckyTargets.value, currentPrize.value)
        prizeConfig.normalizePrizeUsage(currentPrize.value)
        prizeConfig.syncPrizeToList(currentPrize.value)
        prizeConfig.updatePrizeConfig(currentPrize.value)
        await enterLottery()
    }
    /**
