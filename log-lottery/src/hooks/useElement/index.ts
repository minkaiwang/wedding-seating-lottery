...
const LUCKY_HORIZONTAL_GAP = 1.52
const LUCKY_VERTICAL_GAP = 1.28
...
export function useElementStyle(props: IUseElementStyle) {
    const { element, person, index, patternList, patternColor, cardColor, cardSize, scale, textSize, mod, type, textColor } = props
    const luckyTextColor = textColor && textColor !== '#00000000' ? textColor : '#1f2937'
    const luckyDetailColor = '#4b5563'
    const isPatternSlot = patternList.includes(index + 1)
    if (isPatternSlot && mod === 'default') {
        element.style.backgroundColor = rgba(patternColor, Math.random() * 0.2 + 0.8)
    }
    else if (mod === 'sphere' || mod === 'default') {
        // 球体/横铺统一粉色系；图案位在球体上用略深的粉，避免蓝色突兀
        const alpha = isPatternSlot && mod === 'sphere'
            ? Math.random() * 0.15 + 0.62
            : Math.random() * 0.5 + 0.25
        element.style.backgroundColor = rgba(cardColor, alpha)
    }
    else if (mod === 'lucky') {
        element.style.backgroundColor = '#ffffff'
    }
    if (mod === 'lucky') {
        element.style.border = '3px solid #be123c'
        element.style.boxShadow = '0 12px 40px rgba(190, 18, 60, 0.45), 0 0 0 1px rgba(255, 255, 255, 0.9) inset'
    }
    else {
    ...
    element.children[1].style.lineHeight = `${textSize * scale * 1.15}px`
    if (mod === 'lucky') {
        element.children[0].style.color = '#9f1239'
        element.children[0].style.fontWeight = '700'
        element.children[1].style.color = luckyTextColor
        element.children[1].style.fontWeight = '800'
        element.children[1].style.textShadow = '0 1px 0 rgba(255,255,255,0.9), 0 2px 8px rgba(190,18,60,0.25)'
        element.children[2].style.color = luckyDetailColor
        element.children[2].style.fontWeight = '600'
        element.children[2].style.lineHeight = '1.35'
    }
    else {
        const sphereNameColor = '#ffffff'
        const sphereDetailColor = 'rgba(255,255,255,0.92)'
        const sphereIdColor = 'rgba(255,255,255,0.85)'
        element.children[0].style.color = sphereIdColor
        element.children[1].style.color = sphereNameColor
        element.children[1].style.fontWeight = '700'
        element.children[1].style.textShadow = '0 1px 3px rgba(0,0,0,0.45), 0 0 10px rgba(255,255,255,0.2)'
        element.children[2].style.color = sphereDetailColor
        element.children[2].style.fontWeight = '600'
    }
    if (person.name) {
...
export function useElementPosition(
    ...
    // 计算每行的垂直中心位置（y 越大越靠上；z=1000 时需略下移避免与标题重叠）
    const verticalSpacing = scaledCardHeight * LUCKY_VERTICAL_GAP
    const totalHeight = (length - 1) * verticalSpacing + scaledCardHeight
    const headerOffset = Math.min(100, windowSize.height * 0.06)
    const zPerspectiveOffset = 180

    yTable = windowSize.height / 2 - totalHeight + currentRow * verticalSpacing - headerOffset - zPerspectiveOffset
    // 计算当前行的水平居中偏移
    const horizontalSpacing = scaledCardWidth * LUCKY_HORIZONTAL_GAP
    ...
    return { xTable, yTable, scale: scale * LUCKY_CARD_SCALE_BOOST }
}
