...
### A5 static observability correction

Before any holdout execution, static review found that the first H04 patch moved only the metadata trace for successful replacement completion. That candidate changed the observation tool rather than the application's externally visible completion. H04 was corrected, without inspecting any holdout result, to move the actual `WEDDING_SEATING_IMPORT_DONE` success message and its matching trace before persistence and to remove both from their original post-persistence position. Its fault ID, implementation stratum, asynchronous-reordering operator, normal replacement trigger, and expected I4 consequence remain unchanged. The amended catalogue is the only executable holdout selection.

### A5 harness correction after incomplete candidate run

Candidate run `formal_20260826b` completed H01-H07 but produced no complete H08 browser replication. The H08 persistence-call deletion also disabled the live-sync operation that the harness had used to establish its durable pre-state, so each browser stopped during setup before the designated update. Candidate `formal_20260826c` then seeded that state through the unaffected replacement path; Chromium and Firefox completed, but Playwright WebKit isolated the top-level replacement page from the embedded receiver's storage partition. The harness therefore writes the same synthetic 200-record pre-state directly into the IndexedDB partition of the already-loaded live-sync iframe before sending the target update. H08, its trigger payload, expected consequence, baselines, O2 clauses, browsers, and exclusion rules are unchanged. Both incomplete candidates are retained as execution records and excluded from the formal holdout result.
